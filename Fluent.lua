    ...
    [26] = function()
        ...
        function c.New(d, e, f)
            ...
            function h.SetValue(p, s)
                local clamped = math.clamp(s, h.Min, h.Max)
                local stepped = h.Min + math.floor((clamped - h.Min) / h.Increment + 0.5) * h.Increment
                p.Value = g:Round(stepped, h.Rounding)
